...
def A(L, type1, type2, sigma, Delta_T, Delta_P, lmaxint = lmaxintdef, num_samples = 201):
    print(L, type1, type2)
    integrand = lambda lx, ly : A_integrand(lx, ly, L, type1, type2, sigma, Delta_T, Delta_P)

    ls = np.linspace(-1*lmaxint, lmaxint, num_samples)

    # Integrate over lx and ly in the square [-3000, 3000] x [-3000, 3000]
    int_vals = np.array([[integrand(lx, ly) for lx in ls] for ly in ls])

    I_x = simpson(int_vals, 
                  x = ls, 
                  axis = 1
                  )
    integral = simpson(I_x, 
                       x = np.linspace(-1*lmaxint, lmaxint, num_samples)
                       )
...
def N(L, type11, type12, type21, type22, sigma, Delta_T, Delta_P, lmaxint = lmaxintdef, num_samples = 201):
    print(L, type11, type12, type21, type22)
    if type11 == type21 and type12 == type22:
        return A(L, type11, type12, sigma, Delta_T, Delta_P, lmaxint = lmaxint, num_samples = num_samples)
        ...
        I_x = simpson(int_vals, x = ls, axis=1)
        integral = simpson(I_x, x = ls)

        # integral, err = dblquad(integrand, -1*lmaxint, lmaxint,
        #                         lambda lx: -1*lmaxint, lambda lx: lmaxint,
        #                         epsrel=1e-1, epsabs=1e-1)
        
        return L**(-2) * A(L, type11, type12, sigma, Delta_T, Delta_P, lmaxint = lmaxint, num_samples = num_samples) * A(L, type21, type22, sigma, Delta_T, Delta_P, lmaxint = lmaxint, num_samples = num_samples) * (2 * np.pi)**(-2) * integral
...
def config_mat(L, sigma, Delta_T, Delta_P, num_samples, lmaxint):
    ...
    for i, j in itertools.product(range(num_configs), repeat = 2):
        if i >= j:
            result = N(L, *configs[i], *configs[j], sigma, Delta_T, Delta_P, lmaxint = lmaxint, num_samples = num_samples)
            mat[i, j] = result
            mat[j, i] = result
