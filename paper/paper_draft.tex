...
\begin{empheq}[box=\fbox]{align*}
    P^{\psi_X\psi_Y}_l
    & = \frac{9}{l^4}\Omega_m^2H_0^4\int_0^{\chi_*} \chi^2 \d\chi a(\eta_0-\chi)^{-2}W_X(\chi)W_Y(\chi)P^\delta(l/\chi,\eta_0-\chi),\\
...
\end{equation}
where \( P_m \) is the nonlinear matter power spectrum\footnote{Compare to the tree level bispectrum where we instead use the linear powerspectrum.}, and the kernel \( F_2 \) is modified from the tree level result with factors \( a(k, z) \), \( b(k, z) \), and \( c(k, z) \):
\begin{equation}
...
\begin{equation}
    4 \pi k_{\text{NL}}^3 P_m^{\text{lin}}(k_{\text{NL}}, 0) = 1.
\end{equation}
...
Quantities we are interested in measuring depend on how the lensing vector changes with the observation angle. The magnification matrix is defined as
\begin{equation}
...
We can rewrite equations \ref{eq:lpsintermsofpsi} and \ref{eq:lbsintermsofpsi} in terms of the matter spectra instead of the graviational potential spectra using the poisson equation. This then allows us to numerically evaluate these objects using CAMB. The \textbf{density contrast} is defined as
\begin{equation}
...
In the case of a flat universe ($K=0$) we can thus rewrite the Poisson equation \cite{dodelson2020modern} as
