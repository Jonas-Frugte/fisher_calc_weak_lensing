...
TODO: explain what it is, give some references, then explain the advantages.

More information can be extracted from galaxy weak lensing surveys by binning the galaxy population, see figure \ref{fig:galaxyredshiftbinning}. This gives additional lensing power spectra and bispectra. We get auto spectra such as $C^{11}_{\ell}$ and cross spectra such as $C^{12}_{\ell}$. The same is true for the bispectra. The corresponding noise spectra are larger because the population in each bin is of course smaller. Nevertheless, the overall constraining power is usually significantly improved by using such "redshift tomography" (TODO: ADD SOME REFS). The constraining power is especially improved if the different bins are lensed differently, i.e. if the cross-bin spectra are small. One can also take into account the significant uncertainty in galaxy redshift. This is done by convolving the galaxy distribution function with the distribution function of a galaxy's true redshift given its observed redshift. See again figure \ref{fig:galaxyredshiftbinning}.

The obvious question when analyzing (simulated) data is how many bins to use and where their boundaries should be. There is no easy answer for this and different options should be tested for different bin configurations. Using more bins generally leads to increasingly tight constraints but with diminishing returns. For example, in https://arxiv.org/pdf/1306.6870 they find little improvement beyond 4 bins when considering galaxy shear power spectra TODO: CHECK WHAT THEY ARE SPECIFICALLY LOOKING AT AND ADD PROPER REF. Some considerations are:
\begin{enumerate}
    \item If the noise starts to be bigger than the signal for each bin, further improvements in constraints will be minimal. (TODO ADD REF https://arxiv.org/pdf/astro-ph/9904153)
...
\begin{figure}
    ...
    \caption{Our galaxy redshift distribution (TODO: REF) split up into 4 bins. The dashed lines correspond to the actual distributions of the bins after taking into account galaxy redshift uncertainty. We assumed no galaxy bias in the redshift estimation and a gaussian error given by $\sigma_z = 0.05 (1+z)$. This is representative of Euclid level of redshift uncertainty TODO: CONFIRM THIS AND ADD REF.}
...
\end{equation*}
where $X$, $Y$, etc. are the tracers (TODO IS THIS THE CORRECT WORD?) that we correlate. Without redshift tomography we get $X,Y,\cdots\in\{\text{CMB}, \text{gal}\}$ and
\begin{equation*}
    ...
    \end{pmatrix},
\end{equation*}
whereas with redshift tomography we get $X,Y,\cdots\in\{\text{CMB}, \text{gal 1}, \text{gal 2}, \cdots\}$ and a corresponding larger $C_\ell$ matrix.
...
Finally, we also need to take into account baryonic feedback. It has been shown that not taking into account baryonic feedback will lead to a bias of multiple sigma in, for example, $S_8$, $\Omega_m$, $\Omega_b$ when analyzing 2 and 3 point correlations of Euclid mock data (TODO: https://arxiv.org/pdf/2506.18974). We model the effects by using HMCODE2020 and marginalizing over $\log T_{\text{AGN}}$ in addition to the parameters described above. It was shown in CITE: HMCODE2020 that this one parameter allows models of the nonlinear power spectrum to incorporate baryonic feedback at a high level of accuracy (compared to models with more degrees of freedom for baryonic feedback). In this way we are able to make our considerably more realistic with minimal extra computational cost. 
...
Redshift binning / tomography has long been known to improve constraint from galaxy shape surveys (TODO: provide good refs). We expect this to be used in future surveys as well and thus choose to model it in our calculations. The number and width of bins is not an exact science. In theory, the larger the number of bins, the better our constraints. In practice these improvements drop off rapidly after about 5 bins (depending also on the amount of redshift uncertainty). We thus choose to take 4 bins, each with borders placed so that they contain an equal number of galaxies. In order to model redshift uncertainty, we convolve each bin distribution with a redshift dependent gaussian uncertainty $\sigma_z = 0.05 (1+z)$. This is representative of the type of uncertainty we expect from surveys such as EUCLID. (TODO: check formula TODO: add ref) See figure \ref{fig:galaxyredshiftbinning} for the resulting true distributions of each bin.
...
\begin{itemize}
    ...
    \item Although HMCODE2020 models baryonic feedback on the nonlinear matter power spectrum well, it is not guaranteed that we get the same accuracy for modelling feedback effects on the matter bispectrum. This is because it was only tested on the nonlinear matter powerspectrum CITE: HMCODE2020. Clearly, there is good reason to think that an accurate model of the matter powerspectrum will lead to an accurate model of the matter bispectrum due to the fitting equation we use \eqref{eq:matterbispectrum}, but this should be checked in future work. Alternatively, one could use the matter bispectrum emulator from (TODO: CITE https://arxiv.org/pdf/2506.18974), which is specifically made to accurately model baryonic feedback for the scales we expect to probe in a EUCLID-like survey.
