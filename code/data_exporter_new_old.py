...
def data_export(folder_name, cosm_par, lps = True, a_create = True, b_create = True, c_create = True, mps = True, rest = True):
    ...
    # order of inputs: H0, ombh2, omch2, ns, redshifts, As, mnu, logT_AGN
    cosm_par_dict = {
            'H': cosm_par[0],
            'ombh2': cosm_par[1],
            'omch2': cosm_par[2],
    ...
    if rest:
        ...
        np.save(filepath + '/window_func_s1', [cosm_data.window_func(chi, 's1') for chi in chis_log])
        print(f'window_func_s1 created to {folder_name}')
        np.save(filepath + '/window_func_s2', [cosm_data.window_func(chi, 's2') for chi in chis_log])
        print(f'window_func_s2 created to {folder_name}')
        np.save(filepath + '/window_func_s3', [cosm_data.window_func(chi, 's3') for chi in chis_log])
        print(f'window_func_s3 created to {folder_name}')
        np.save(filepath + '/window_func_s4', [cosm_data.window_func(chi, 's4') for chi in chis_log])
        print(f'window_func_s4 created to {folder_name}')
    ...
    if lps:
        ...
        print(f'lensing power spectrum created to {folder_name}, dim: {np.dim(lps_data)}')
        # for tracer1 in ('c', 's1', 's2', 's3', 's4'):
        #     for tracer2 in ('c', 's1', 's2', 's3', 's4'):
        #         # lensing power spectrum (convergence, convergence) (1d: k)
        #         np.save(filepath + '/lensing_power_spectrum_' + tracer1 + tracer2, [cosm_data.lps(k, (tracer1, tracer2)) for k in ks_log_fine])
        #         print(f'lensing power spectrum {tracer1}{tracer2} created to {folder_name}')
