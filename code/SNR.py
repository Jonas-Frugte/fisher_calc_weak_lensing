...
def single_bs_SNR2():
    ...
    data = [Fisher_mat_single(lmin, ls[1-1], ls[1], stepsize, 100, b'snr', b'snr', num_cores, b'c')]
    for i in range(2, len(ls)):
        data.append(data[-1] + Fisher_mat_single(lmin, ls[i-1], ls[i], stepsize, 100, b'snr', b'snr', num_cores, b'c'))
    print(data)

    print("Shear bispectrum SNR^2:")

    data = [Fisher_mat_single(lmin, ls[1-1], ls[1], stepsize, 100, b'snr', b'snr', num_cores, b's')]
    for i in range(2, len(ls)):
        data.append(data[-1] + Fisher_mat_single(lmin, ls[i-1], ls[i], stepsize, 100, b'snr', b'snr', num_cores, b's'))
    print(data)
