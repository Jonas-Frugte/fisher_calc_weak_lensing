...
gal_lps_bin1 = np.array([spectra.lps(l, ('s1', 's1')) for l in l_values])
gal_lps_bin4 = np.array([spectra.lps(l, ('s4', 's4')) for l in l_values])
cmb_lps = np.array([spectra.lps(l, ('c', 'c')) for l in l_values])

# CMB NOISE QUAD ESTIMATOR
ls_cmbn = np.loadtxt('cmb_noise_files/ls_1_3000_32.txt')
...
n_g_S3 = 0.25 * 5 / arcmin2_to_sr
n_g_S4 = 0.25 * 30 / arcmin2_to_sr
...
axs[0].loglog(l_values, l_values**4 * gal_lps_bin1, label='Galaxy lensing potential powerspectrum, bin 1', color='black')
axs[0].loglog(l_values, l_values**4 * gal_lps_bin4, label='Galaxy lensing potential powerspectrum, bin 4', color='gray')
    
axs[0].loglog(l_values, l_values**4 * gal_nps_S3, label=r'S3 Noise, $n_g=5 / 4\,\mathrm{arcmin}^{-2}$', linestyle='--')
axs[0].loglog(l_values, l_values**4 * gal_nps_S4, label=r'S4 Noise, $n_g=30 / 4\,\mathrm{arcmin}^{-2}$', linestyle='--')
...
plt.savefig("plots/spectraplusnoise_new.png", dpi=300, bbox_inches="tight")
plt.savefig("plots/spectraplusnoise_new.pdf", dpi=300, bbox_inches="tight")
